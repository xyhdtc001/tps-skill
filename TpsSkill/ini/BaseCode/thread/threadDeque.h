...
class ThreadSafeDeque
{
   ...
   public:
      ...
      {
         #ifdef _DEBUG
         log_out( MAIN, LOG_DEBUG, "[ThreadSafeDeque] numValues=%i", mNumValues );
         mFreeList.dumpDebug();
         #endif
      }
};
...
void ThreadSafeDeque< T >::pushFront( const ValueType& value )
{
   ...
   while( 1 )
   {
      ...
      else if (!nextNode->mNext.isTagged())
      {
...
void ThreadSafeDeque< T >::pushBack( const ValueType& value )
{
   ...
   while( 1 )
   {
      ...
      else if (!prevNode->mPrev.isTagged())
      {
