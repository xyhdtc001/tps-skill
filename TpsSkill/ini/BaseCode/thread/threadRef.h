...
class ThreadSafeRefCount
{
   ...
   protected:
      ...
      uint32            mRefCount;  ///< Reference count and claim bit.  Note that this increments in steps of two.

      static uint32     decrementAndTestAndSet( uint32& refCount );
};
...
{
   ASSERT_MESSAGE( mRefCount != 0, "ThreadSafeRefCount::release() - refcount of zero" );
...
template< class T, class DeletePolicy >
uint32 ThreadSafeRefCount< T, DeletePolicy >::decrementAndTestAndSet( uint32& refCount )
{
   uint32 oldVal;
   uint32 newVal;
...
{
   ASSERT_MESSAGE( mRefCount % 2 != 0, "ThreadSafeRefCount::clearLowestBit() - invalid refcount" );
