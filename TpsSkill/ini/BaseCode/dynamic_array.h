...
private:
	...
	T* allocate (size_t size)
	{
		...
		ASSERT_MESSAGE(sizeof(T) != 0, "incomplete type");
		ASSERT_MESSAGE(align != 0, "incomplete type");

		return static_cast<T*> (AlignedMemory_allocate (/*m_label,*/ size * sizeof(T), align));
	}

	T* deallocate (T* data)
	{
		ASSERT(owns_data());
		AlignedMemory_deallocate (/*m_label, */data);
		return NULL;
	}
	...
	T* reallocate (T* data, size_t size)
	{
		...
		return static_cast<T*> (AlignedMemory_realloc(/*m_label, */data, size * sizeof(T), alignof));
	}
